...
const DocumentAnalyzer: React.FC<DocumentAnalyzerProps> = ({ onBack }) => {
  ...
  const handleAnalyze = async () => {
    ...
    reader.onload = async () => {
      ...
      const result = await analyzeDocument(base64String, file.type);
      setAnalysis(result);
      setLoading(false);
    };
    reader.onerror = () => {
        setAnalysis("Error reading file.");
        setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col h-full bg-darkbg">
       <div className="p-4 bg-cardbg border-b border-slate-700 flex items-center space-x-3">
        ...
        <h2 className="text-lg font-bold text-white">Document Analyzer</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        {!file ? (
          <div className="border-2 border-dashed border-slate-600 rounded-xl p-8 flex flex-col items-center justify-center text-slate-400 bg-cardbg/50">
            ...
            <p className="mb-2 text-center">Upload Legal Document (PDF/Image)</p>
            <p className="text-xs text-slate-500 text-center mb-6">Contracts, Notices, FIR copies, etc.</p>
        ...
        {analysis && (
          <div className="bg-cardbg rounded-xl p-5 border border-slate-700 shadow-lg">
            <h3 className="text-lg font-bold text-green-400 mb-3 flex items-center">
                <ShieldIcon className="mr-2" size={20} /> Analysis Result
            </h3>
...
// Helper icon since Shield is used inside
